...
### 阶段五：模拟交易增强：投资组合分析与基础风险管理

*   **总目标**: 进一步增强模拟交易系统的真实性和实用性，引入更细致的投资组合表现分析能力和初步的风险控制机制，加深对投资组合管理和风险控制基本概念的理解。
*   **总体状态**: `[进行中]`
*   **主要模块与任务**:
    *   **5.1 增强型投资组合分析 (模拟环境)**:
        *   `[✅]` **5.1.1 多维度盈亏计算与展示**:
            *   **学习内容**: 理解并区分已实现盈亏 (Realized P&L) 和未实现盈亏 (Unrealized P&L / Mark-to-Market)，这是评估投资表现的核心。学习跟踪交易成本对利润的影响。
            *   **任务**: 在 `MockPortfolio` 中实现对已实现盈亏（交易平仓时计算）和未实现盈亏（基于当前市价与平均成本价计算）的跟踪与累计。在Web UI (`SimulationDisplay`) 中清晰展示这两类盈亏以及总盈亏。
        *   `[✅]` **5.1.2 多品种持仓分析与可视化**:
            *   **学习内容**: 理解资产配置的基本概念，以及如何评估和监控投资组合中不同资产的集中度风险。
            *   **任务**:
                *   确保 `MockPortfolio` 能统一管理多个不同股票代码的持仓（已具备基础）。
                *   在UI中为每个持仓品种展示其数量、平均成本价、当前市价、市值及未实现盈亏。
                *   计算并展示各品种市值占总投资组合净值（现金+总持仓市值）的百分比，考虑在UI上通过简单图表（如文字列表或条形图）进行示意。
    *   **5.2 基础风险控制规则实现 (模拟环境)**:
        *   **学习内容**: 理解不同类型风险控制措施（如止损、持仓限制、最大回撤）的意义、实现方式及其在实际交易中的作用。学习风险事件的初步监控与响应。
        *   **任务**:
            *   `[ ]` **5.2.1 单个头寸风险控制 - 止损线 (Stop-Loss for a Position)**: 实现当某个持仓的未实现亏损达到预设的金额或其平均买入成本的百分比时，触发告警和日志记录。
            *   `[ ]` **5.2.2 品种级别风险控制 - 最大持仓限制 (Max Position Size per Asset)**: 实现当单个股票的持仓市值（或数量，初期可简化为市值）超过总投资组合净值的预设百分比时，触发告警和日志记录。
            *   `[ ]` **5.2.3 账户级别风险控制 - 最大回撤 (Max Drawdown from Peak)**: 实现当账户总净值从模拟会话开始以来记录到的最高点回落达到预设百分比或金额时，触发告警和日志记录。
            *   `[⚙️]` **5.2.4 风险规则配置与触发处理**: 初期，风险参数将在后端硬编码或通过简单配置管理。风险规则的检查点设在交易执行前或市场价格更新后。触发时，主要操作为详细日志记录并在Web UI上显示明确的告警信息。 **(UI展示已完成)**
*   **学习重点**: 投资组合理论（盈亏计算、多资产管理、资产配置概念），风险指标（如最大回撤的理解与监控），风险控制措施（止损、持仓上限）的原理与模拟实现，系统状态管理与事件处理逻辑。
